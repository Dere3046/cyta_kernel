...
void verify_kallsyms(void)
{
	...
	/*
	 * bootstrap complete. from here on, call the kernel's own
	 * kallsyms_lookup_name directly instead of using our binary
	 * search. cache the pointer after init:
	 *
	 *   static unsigned long (*klp)(const char *);
	 *   klp = (void *)kallsyms_name_to_addr("kallsyms_lookup_name");
	 *   klp("sys_call_table");
	 *
	 * the init cost is paid once, all subsequent lookups are O(1).
	 */

#if 0
	/* deep probe trace — keep for debugging */

	/* trace binary search path */
	int lo = 0, hi = (int)klnum_val - 1;
	while (lo <= hi) {
		int mid = lo + (hi - lo) / 2;
		unsigned int s = get_sym_seq(mid);
		unsigned int o = get_sym_offset(s);
		char nb[256];
		expand_sym(o, nb, sizeof(nb));
		int r = strcmp("kallsyms_lookup_name", nb);
		pr_info("[ksymless] binsearch: lo=%d hi=%d mid=%d seq=%u off=%u cmp=%d name='%s'",
			lo, hi, mid, s, o, r, nb);
		if (r > 0)
			lo = mid + 1;
		else if (r < 0)
	...
	/* trace first 5 symbols */
	for (int i = 0; i < 5; i++) {
		unsigned int s = get_sym_seq(i);
		unsigned long a = sym_addr(s);
		char tn[256];
		sprint_symbol_no_offset(tn, a);
		unsigned int o = get_sym_offset(s);
		expand_sym(o, our, sizeof(our));
		pr_info("[ksymless] trace[%d]: seq=%u addr=0x%lx sp='%s' our='%s' %s\n",
			i, s, a, tn, our,
			strcmp(tn, our) == 0 ? "OK" : "FAIL");
	}
#endif
}
